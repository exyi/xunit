...
public class XunitDelayEnumeratedTheoryTestCaseRunner : XunitTestCaseRunnerBase<XunitDelayEnumeratedTheoryTestCaseRunnerContext>
{
	...
	protected override async ValueTask AfterTestCaseStartingAsync(XunitDelayEnumeratedTheoryTestCaseRunnerContext ctxt)
	{
		...
		try
		{
			...
			foreach (var dataAttribute in dataAttributes)
			{
				...
				try
				{
					discoverer = ExtensibilityPointFactory.GetDataDiscoverer(ctxt.DiagnosticMessageSink, discovererAttribute);
					if (discoverer == null)
					{
						if (dataAttribute is _IReflectionAttributeInfo reflectionAttribute)
	...
	/// <param name="diagnosticMessageSink">The message used to send diagnostic messages to.</param>
		...
		_IMessageSink diagnosticMessageSink)
	{
		...
		return RunAsync(new(testCase, messageBus, aggregator, cancellationTokenSource, displayName, skipReason, testClass, constructorArguments, testMethod, testMethodArguments, beforeAfterTestAttributes, diagnosticMessageSink));
	}

	/// <inheritdoc/>
	protected override async ValueTask<RunSummary> RunTestAsync(XunitDelayEnumeratedTheoryTestCaseRunnerContext ctxt)
	{
