	...
	where TTestCase : class, _ITestCase
{
	...
	public async ValueTask<RunSummary> RunAsync(TContext ctxt)
	{
		...
		else
		{
			try
			{
				...
				summary = await RunTestAsync(ctxt);
	...
	/// <param name="ctxt">The context that describes the current test case</param>
	/// <returns>Returns summary information about the tests that were run.</returns>
	protected abstract ValueTask<RunSummary> RunTestAsync(TContext ctxt);
